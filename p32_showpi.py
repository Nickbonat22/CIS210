'''
Show Monte Pi
CIS 210 F17 Project 3, Part 2

Author: Nicholas Bonat

Credits: Based on code on p.78 Miller and Ranum text.

This project will determine an approximate value of pi using the Monte Carlo
algorithm and draw it.
'''

from turtle import *
import math
import random

def showMontePi(numDarts):
    '''(num) -> float

    numDarts is the parameter representing how many times the pi process
    should run. A pi approximation is calucalted while darts are drawn using
    turtle graphics. The approximation is returned and a report is printed.

    >>> montePi(100)
    3.0
    >>> montePi(100000)
    3.143072
    '''

    wn = Screen()

    wn.setworldcoordinates(-2,-2,2,2)

    up()
    goto(-1,0)
    down()
    goto(1,0)

    up()
    goto(0,1)
    down()
    goto(0,-1)

    circleCount = 0
    up()

    for i in range(numDarts):
        x = random.random()
        y = random.random()

        inCircle = isInCircle(x,y,1)

        goto(x,y)

        if inCircle:
            circleCount = circleCount + 1
            color("blue")
        else:
            color("red")

        dot()
        speed(0)

    pi = circleCount / numDarts * 4

    #click on turtle window to view report
    wn.exitonclick()

    lib_pi = math.pi

    error = (lib_pi - pi)

    percent_error = (error/lib_pi) * 100

    percent_error1 = ("%.2f" % abs(percent_error))

    print ("With",numDarts,"iterations:",
           "\nmy approximation value for pi is:",pi,
           "\nmath lib pi value is:",lib_pi,
           "\nThis is a",percent_error1,"percent error\n")
           
    return pi

def isInCircle(x,y,r):
    '''
    num -> boolean

    The x and y parameters are the values of the point and r represents the
    radius. This function will return True if the input value is inside the
    circle and False if otherwise.

    >>> isInCircle(0,0,1)
    True
    >>> isInCircle(1,2,1)
    False
    '''

    if math.sqrt(x ** 2 + y ** 2) < r * r:
        return True
    else:
        return False
    
